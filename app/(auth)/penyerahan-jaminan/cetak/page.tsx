...
export default function Page() {
  ...
  const [pencairan, setPencairan] =
    useState<IPenyerahanJaminan>(defaultJaminan);
  ...
  const getData = async () => {
    ...
    const res = await fetch(`/api/penyerahan-jaminan?${params.toString()}`);
  ...
  const generateNoSI = async () => {
    ...
    await fetch("/api/penyerahan-jaminan?id=" + sumdan.id, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((res) => {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    await fetch("/api/penyerahan-jaminan", {
      method: "POST",
  ...
  return (
    <div>
      <Card
        title={
          <div>
            <SafetyCertificateOutlined /> Cetak Penyerahan Jaminan
          </div>
        }
...
const defaultJaminan: IPenyerahanJaminan = {
  id: "",
